...
RUN echo "- Flutter"; \
    ...
    \
    android_studio_out=androidstudio.tgz; \
    wget -q https://redirector.gvt1.com/edgedl/android/studio/ide-zips/4.0.0.16/android-studio-ide-193.6514223-linux.tar.gz -O $android_studio_out; \
    tar -C /usr/local -xf $android_studio_out; \
    \
...
ENV ANDROID_HOME /user/local/android-studio
ENV PATH /usr/local/flutter/bin:/usr/local/android-studio/bin:$PATH
